...
def board_to_dataframe(board: BoardData) -> pd.DataFrame:
    ...
    for item in board.items:
        row: dict[str, str] = {"__item_id": item.get("id", ""), "__item_name": item.get("name", "")}
        by_title = {cv["column"]["title"]: (cv.get("text") or "") for cv in item.get("column_values", [])}
        for title in col_titles:
            row[title] = by_title.get(title, "")
        rows.append(row)
    df = pd.DataFrame(rows, columns=["__item_id", "__item_name", *col_titles])
    return df
